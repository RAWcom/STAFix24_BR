...
namespace EventReceivers.tabKlienci
{
    ...
    public class tabKlienci : SPItemEventReceiver
    {
        ...
        private void SetUp(SPListItem item)
        {
            ...
            item["Title"] = BLL.Tools.Get_Text(item, "_NazwaPrezentowana");
        ...
        {
            Update_Serwisy_FP(item);
            Update_Serwisy_WSO(item);
        ...
        private void Update_Serwisy_WSO(SPListItem item)
        {
            BLL.Tools.Remove_Services(ref item, "selSewisy", "WSO");
            ...
            switch (item.ContentType.Name)
            {
                ...
                case "Firma wewnętrzna":
                    ...
                    if (BLL.Tools.Get_LookupId(item, "selKlient_NazwaSkrocona")>0
                        && BLL.Tools.Get_Value(item, "colPD_UdzialWZysku")>0)
                    {
                        BLL.Tools.Assign_Service(ref item, "selSewisy", "WSO");
                        break;
                    }
        ...
        /// forma prawna
        /// </summary>
        private void Update_Serwisy_FP(SPListItem item)
        {
            BLL.Tools.Remove_Services(ref item, "selSewisy", "FP-*");

            switch (item.ContentType.Name)
            {
                case "Klient":

                    if (BLL.Tools.IsSelectorAssigned(item, "enumFormaPrawna", string.Empty))
                    {
                        switch (BLL.Tools.Get_Text(item, "enumFormaPrawna"))
                        {
                            case "Jednoosobowa działalność gospodarcza":
                                BLL.Tools.Assign_Service(ref item, "selSewisy", "FP-DG");
                                break;
                            case "Spółka z o.o.":
            ...
            switch (item.ContentType.Name)
            {
                ...
                case "Firma zewnętrzna":
                    ...
                    if (BLL.Tools.IsSelectorAssigned(item, "selUrzadSkarbowy", string.Empty)
                        && BLL.Tools.IsSelectorAssigned(item, "colFormaOpodatkowaniaPD", "Nie dotyczy")
